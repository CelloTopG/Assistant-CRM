...
def after_migrate():
    ...
    try:
        ...
        # Apply Company → Employer rename via Property Setters
        try:
            setup_employer_rename()
        except Exception as e:
            frappe.log_error(f"after_migrate employer rename setup error: {str(e)}", "Assistant CRM Install")
...
def setup_employer_rename():
    """Setup Property Setters to rename Company → Employer in the UI.

    This is idempotent - can be run multiple times safely.
    """
    from frappe.custom.doctype.property_setter.property_setter import make_property_setter

    # Property setters for Company doctype label changes
    property_setters = [
        # Change the doctype's display name in various places
        {
            "doctype": "Company",
            "fieldname": None,
    ...
    for ps in property_setters:
        try:
            make_property_setter(
                ...
                ps["value"],
                ps["property_type"],
                for_doctype=ps["fieldname"] is None
            )
        except Exception as e:
            frappe.log_error(f"Error setting property {ps}: {str(e)}", "Employer Rename")

    # Update the Module Def to show "Employer" instead of "Company" in sidebar
    # This is done via translation, not property setter

    # Create/update translation entries in the database
    _ensure_employer_translations()

    frappe.db.commit()
    print("✅ Company → Employer UI rename applied")


def _ensure_employer_translations():
    """Ensure Company → Employer translations are in the database.

    This makes the translations persistent and available across sessions.
    """
    import csv
    import os
    ...
    with open(translations_path, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            if source and translated:
                translations.append((source, translated))

    # Insert/update translations in the Translation doctype
    ...
    frappe.cache().delete_key("lang_full_dict::en")
